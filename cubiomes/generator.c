...
CBHD void setupGenerator(Generator *g, int mc, uint32_t flags)
{
    g->mc = mc;
    g->dim = DIM_UNDEF;
    g->flags = flags;
...
CBHD void applySeed(Generator *g, int dim, uint64_t seed)
{
    g->dim = dim;
    g->seed = seed;
    g->sha = 0;
...
CBHD int mapApproxHeight(float *y, int *ids, const Generator *g, const SurfaceNoise *sn,
