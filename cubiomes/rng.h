...
#ifdef __CUDA_ARCH__
#define BSWAP32(X)              __byte_perm((uint32_t)(X), 0u, 0x0123u)
#else
#define BSWAP32(X)              __builtin_bswap32(X)
#endif
#ifdef __CUDA_ARCH__
#define UNREACHABLE()           __trap()
#else
...
static CBHD inline ATTR(const, always_inline, artificial)
...
static CBHD inline ATTR(const, always_inline)
...
static CBHD inline void setSeed(uint64_t *seed, uint64_t value)
{
    *seed = (value ^ 0x5deece66d) & ((1ULL << 48) - 1);
}

static CBHD inline int next(uint64_t *seed, const int bits)
{
    *seed = (*seed * 0x5deece66d + 0xb) & ((1ULL << 48) - 1);
    return (int) ((int64_t)*seed >> (48 - bits));
}

static CBHD inline int nextInt(uint64_t *seed, const int n)
{
    int bits, val;
    const int m = n - 1;
...
static CBHD inline uint64_t nextLong(uint64_t *seed)
{
    return ((uint64_t) next(seed, 32) << 32) + next(seed, 32);
}

static CBHD inline float nextFloat(uint64_t *seed)
{
    return next(seed, 24) / (float) (1 << 24);
}

static CBHD inline double nextDouble(uint64_t *seed)
{
 ...
 */
static CBHD inline void skipNextN(uint64_t *seed, uint64_t n)
{
    uint64_t m = 1;
    uint64_t a = 0;
...
static CBHD inline void xSetSeed(Xoroshiro *xr, uint64_t value)
{
...
static CBHD inline uint64_t xNextLong(Xoroshiro *xr)
{
    uint64_t l = xr->lo;
    uint64_t h = xr->hi;
...
static CBHD inline int xNextInt(Xoroshiro *xr, uint32_t n)
{
...
static CBHD inline double xNextDouble(Xoroshiro *xr)
{
    return (xNextLong(xr) >> (64-53)) * 1.1102230246251565E-16;
}

static CBHD inline float xNextFloat(Xoroshiro *xr)
{
    return (xNextLong(xr) >> (64-24)) * 5.9604645E-8F;
}

static CBHD inline void xSkipN(Xoroshiro *xr, int count)
{
    while (count --> 0)
        xNextLong(xr);
}

static CBHD inline uint64_t xNextLongJ(Xoroshiro *xr)
{
...
static CBHD inline int xNextIntJ(Xoroshiro *xr, uint32_t n)
{
    int bits, val;
    const int m = n - 1;
...
static CBHD inline uint64_t mcStepSeed(uint64_t s, uint64_t salt)
{
    return s * (s * 6364136223846793005ULL + 1442695040888963407ULL) + salt;
}

static CBHD inline int mcFirstInt(uint64_t s, int mod)
{
...
static CBHD inline int mcFirstIsZero(uint64_t s, int mod)
{
    return (int)(((int64_t)s >> 24) % mod) == 0;
}

static CBHD inline uint64_t getChunkSeed(uint64_t ss, int x, int z)
{
    uint64_t cs = ss + x;
    cs = mcStepSeed(cs, z);
    cs = mcStepSeed(cs, x);
 ...
 */
static CBHD inline double lerp(double part, double from, double to)
{
    return from + part * (to - from);
}

static CBHD inline double lerp2(
        double dx, double dy, double v00, double v10, double v01, double v11)
{
    return lerp(dy, lerp(dx, v00, v10), lerp(dx, v01, v11));
}

static CBHD inline double lerp3(
...
static CBHD inline double clampedLerp(double part, double from, double to)
{
